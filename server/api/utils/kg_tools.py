...
class HuatuoKnowledgeEmbeddingWrapper(KWrapper):
    ...
    def __init__(self,base_data_path) -> None:
        # 检索embedding模型
        self.top_k = 1
        self.retrieve_embedding_models = SentenceModel('/mnt/nvme1n1/huggingface_cache/models/shibing624/text2vec-base-chinese-sentence')#os.environ['EMBEDDING_PATH']
    ...
    # 查询相关的知识  得到语义相似的问题
    def query_related_context(self, question):
        ...
        for hit in hits[0]:
            ...
            print(self.retrieve_corpus[hit['corpus_id']], "(Score: {:.4f})".format(hit['score']))
...
rag_dict = {
    ...
    'embedding_lgph' : HuatuoKnowledgeEmbeddingWrapper(base_data_path=os.path.join(base_path,'longgang_hospitals/longgang_renmin_hospital/longgang_people_hospital.json'))
}
...
if __name__ == '__main__':
    while True:
        ...
        results = rag_dict.get('embedding_lgph').wrap_question(question)
        print(results)
